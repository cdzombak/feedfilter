...
.PHONY: package
package: ## Create .deb packages for amd64, arm64, and armhf
	$(MAKE) package-deb-amd64 package-deb-arm64 package-deb-armhf

.PHONY: package-deb-amd64
package-deb-amd64: build-linux-amd64 ## Create .deb package for amd64
	fpm -s dir -t deb -n ${BIN_NAME} -v ${BIN_VERSION} \
		-p ./out/${BIN_NAME}_${BIN_VERSION}_amd64.deb \
		-a amd64 \
		./out/${BIN_NAME}-${BIN_VERSION}-linux-amd64=/usr/bin/${BIN_NAME}

.PHONY: package-deb-arm64
package-deb-arm64: build-linux-arm64 ## Create .deb package for arm64
	fpm -s dir -t deb -n ${BIN_NAME} -v ${BIN_VERSION} \
		-p ./out/${BIN_NAME}_${BIN_VERSION}_arm64.deb \
		-a arm64 \
		./out/${BIN_NAME}-${BIN_VERSION}-linux-arm64=/usr/bin/${BIN_NAME}

.PHONY: package-deb-armhf
package-deb-armhf: build-linux-armv7 ## Create .deb package for armhf
	fpm -s dir -t deb -n ${BIN_NAME} -v ${BIN_VERSION} \
		-p ./out/${BIN_NAME}_${BIN_VERSION}_armhf.deb \
		-a armhf \
		./out/${BIN_NAME}-${BIN_VERSION}-linux-armv7=/usr/bin/${BIN_NAME}